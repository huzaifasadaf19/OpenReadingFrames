...
def main():
    ...
    parser.add_argument('--print_length', action='store_true', help='Print only the length of the longest ORF')
    ...
    if args.print_length:
        # Print only the length of the longest ORF for each sequence
        for seq_id, longest_orf in result.items():
            print(f"Sequence ID: {seq_id}, Length of Longest ORF: {len(longest_orf)}")

        # Overall longest ORF length
        overall_longest_orf = orf_finder.find_overall_longest_orf(fasta_file_path)
        print(f"Overall Length of Longest ORF: {len(overall_longest_orf)}")
    else:
        # Print the full details as before
        for seq_id, longest_orf in result.items():
            print(f"Sequence ID: {seq_id}, Longest ORF: {longest_orf}")
        
        # Overall longest ORF
        overall_longest_orf = orf_finder.find_overall_longest_orf(fasta_file_path)
        print(f"Overall longest ORF: {overall_longest_orf}")
